...
(define (key-helper n get-nth-quadruple)
  (let* ((quadruple (get-nth-quadruple n))
         (e (cadr quadruple)) (f (caddr quadruple)))
    (map (λ (x) (modulo x 27)) (list (a1 e f) (b1 e f) (c1 e f)
                                       (a2 e f) (b2 e f) (c2 e f)
                                       (a3 e f) (b3 e f) (c3 e f)))))

(define (key n)
  (key-helper n get-nth-quadruple))
...
(define (message->codes-helper message L)
  (cond ((= 0 (length message))
         L)
        ((char-whitespace? (car message))
         (message->codes-helper (cdr message) (append L '(0))))
         ((message->codes-helper (cdr message) (append L (list (- (char->integer (car  message)) 96)))))))

(define (message->codes message)
  (message->codes-helper (string->list message) '()))
...
(define (codes->message-helper codes L)
  (cond ((= 0 (length codes))
         (list->string L))
        ((= 0 (car codes))
         (codes->message-helper (cdr codes) (append L (list (integer->char (+ (car codes) 32))))))
         ((codes->message-helper (cdr codes) (append L (list (integer->char (+ (car codes) 96))))))))

(define (codes->message codes)
  (codes->message-helper codes '()))
...
(define (extend-key key size)
  (let loop ((l key) (dimension size) (result key))
    (if (<= dimension (length result))
        (take result dimension)
        (loop (append (cdr l) (list (car l))) dimension (append result (list (car l)))))))
...
(define (encrypt-decrypt-general l key f)
  (map (λ(x) (modulo x 27)) (map f l key)))
...
(define (encrypt-codes l key)
  (encrypt-decrypt-general l (extend-key key (length l)) +))
...
(define (encrypt-message characters key)
  (codes->message(encrypt-decrypt-general (message->codes characters)
                           (extend-key key (length (message->codes characters))) +)))
(define (decrypt-message characters key)
  (codes->message(encrypt-decrypt-general (message->codes characters)
                           (extend-key key (length (message->codes characters))) -)))
