...
(define ppt-stream-in-tree-order
  (let loop ((Q (stream '(3 4 5))))
    (stream-cons (stream-first Q)
                 (loop (stream-append (stream-rest Q)
                                      (stream (multiply T1 (stream-first Q)))
                                      (stream (multiply T2 (stream-first Q)))
                                      (stream (multiply T3 (stream-first Q))))))))
...
(define (pairs G H)
  (let loop ((G G) (copy-G G) (H H) (cnt 1) (cnt2 1))
    (if (= cnt 0)
        (loop copy-G copy-G (stream-rest H) (+ cnt (+ cnt2 1)) (+ cnt2 1))
        (stream-cons (cons (stream-first G) (stream-first H)) (loop (stream-rest G) copy-G H (- cnt 1) cnt2)))))
...
(define (g-pairs-stream-helper G)
  (let loop ((G G))
    (stream-cons G (loop (+ G 2)))))
    
(define (h-pairs-stream-helper H)
  (let loop ((H H))
    (stream-cons H (loop (+ H 2)))))

(define gh-pairs-stream
  (stream-filter (λ (x)
                   (if (= 1 (gcd (car x) (cdr x)))
                       #t
                       #f)) (pairs (g-pairs-stream-helper 1) (h-pairs-stream-helper 3))))

; TODO
; Definiți fluxul de TPP corespunzător fluxului anterior de
; perechi (g, h).
(define (ppt-stream-helper stream)
  (let loop ((stream-pairs stream))
    (stream-cons (list (* (car (stream-first stream-pairs)) (cdr (stream-first stream-pairs)))
                       (quotient (- (* (cdr (stream-first stream-pairs)) (cdr (stream-first stream-pairs))) (* (car (stream-first stream-pairs)) (car (stream-first stream-pairs)))) 2)
                       (quotient (+ (* (cdr (stream-first stream-pairs)) (cdr (stream-first stream-pairs))) (* (car (stream-first stream-pairs)) (car (stream-first stream-pairs)))) 2))
                 (loop (stream-rest stream-pairs)))))
